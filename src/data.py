...
def load_train(spark):
    ...
    raw_train_df = (spark.read.text(TRAIN_FILE).rdd
                    .map(split_train_row)).toDF(['reuters_id', 'features'])
    ...
    # topics.
    rich_train_df = \
        topics_df.join(raw_train_df, ['reuters_id'])
    ...
    # 3. Undo the sparsify matrix
    rich_train_df = rich_train_df \
        .withColumn('target', (rich_train_df.topic_tag == 'CCAT').cast('integer')) \
        .groupBy(['reuters_id', 'features']).agg({'target': 'sum'}) \
        .rdd \
        .map(sparse_to_dense) \
        .toDF(['reuters_id', 'features', 'target'])

    rich_train_df.printSchema()

    val_df, train_df = \
        rich_train_df.randomSplit([VAL_SPLIT, 1-VAL_SPLIT], seed=24)
...
def split_train_row(row):
    # Split a row of the train dataset.
